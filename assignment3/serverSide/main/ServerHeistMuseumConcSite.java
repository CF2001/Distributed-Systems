 ...
 */
public class ServerHeistMuseumConcSite
{
    ...
    public static void main (String[] args)
    {
        ...
        ConcentrationSite concentrationSite = new ConcentrationSite(reposStub); // concentration site object
        ConcSiteInterface concSiteStub = null;                                  // remote reference to the concentration site object

        try
        { 
            concSiteStub = (ConcSiteInterface) UnicastRemoteObject.exportObject (concentrationSite, portNumb);
        }
        catch (RemoteException e)
        ...
        try
        { while (!end)
            synchronized (Class.forName ("serverSide.main.ServerHeistMuseumConcSite"))
            { try
            { (Class.forName ("serverSide.main.ServerHeistMuseumConcSite")).wait ();
            }
        ...
        { GenericIO.writelnString ("The data type ServerHeistMuseumConcSite was not found (blocking)!");
        e.printStackTrace ();
        System.exit (1);
        }
        ...
        { GenericIO.writelnString ("Concentracion Site deregistration exception: " + e.getMessage ());
            e.printStackTrace ();
            System.exit (1);
        }
        catch (NotBoundException e)
        { GenericIO.writelnString ("Concentracion Site  not bound exception: " + e.getMessage ());
            e.printStackTrace ();
            System.exit (1);
        }
    ...
    public static void shutdown ()
    {
        ...
        { 
            synchronized (Class.forName ("serverSide.main.ServerHeistMuseumConcSite"))
            { 
                (Class.forName ("serverSide.main.ServerHeistMuseumConcSite")).notify ();
            }
        }
        catch (ClassNotFoundException e)
        { 
            GenericIO.writelnString ("The data type ServerHeistMuseumConcSite was not found (waking up)!");
            e.printStackTrace ();
            System.exit (1);
        }
    }
